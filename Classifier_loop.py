...
#wehere to training/validation graphs
...
loop = False
# use pretrained on imagenet?
pretrain = False
#extra save name information
extra_name = '_natural-95p_'
...
if loop == True:
    ...
    for model_type in model_types:
        ...
        def train_model(model, dataloaders, criterion, optimizer, num_epochs=num_epochs, is_inception=False):
            ...
            return model, val_acc_history, best_acc, val_acc
        ...
        # Train and evaluate
        model_ft, hist, best_acc, val_acc = train_model(model_ft, dataloaders_dict, criterion, optimizer_ft, num_epochs=num_epochs, is_inception=(model_name=="inception"))
        model_best.append(model_type)
        ...
        plt.plot(val_loss)
        val_loss_plt.savefig(results + '/' + save_name + '_val-loss.jpg')
        val_acc_plt = plt.figure()
        plt.plot(hist)
        val_acc_plt.savefig(results + '/' + save_name + '_val-acc.jpg')
    ...
    # Train and evaluate
    model_ft, hist, val_loss, train_acc, train_loss = train_model(model_ft, dataloaders_dict, criterion, optimizer_ft, num_epochs=num_epochs, is_inception=(model_name=="inception"))
    ##SAVE MODEL
    save_name = PATH + '/' + model_name + extra_name + '.pt'
    ...
    train_acc_plt = plt.figure()
    plt.plot(train_acc)
    train_acc_plt.savefig(results + '/' + model_name + extra_name + '_train-acc.jpg')

    train_loss_plt = plt.figure()
    plt.plot(train_loss)
    train_loss_plt.savefig(results + '/' + model_name + extra_name + '_train-loss.jpg')
